...
_Local production-build WebKit capture, 10 September 2026, using only the repository’s synthetic demo data. The visible build identity is local evidence only; this is not native Safari, a hosted service, or a release._
...
- keep a Disabled rehearsal available as a read-only local record for document and original-file download until refresh or an explicitly confirmed fresh rehearsal clears it;
...
The production build contains PWA infrastructure and a prompted service-worker update flow. Changed in-memory rehearsals activate common reload protection; a waiting build is blocked during Owner actions or active file preparation/approval, requires an explicit **Update and clear session** decision, and restores that decision if the Update Handoff times out or the prior page returns. Each application and generated service worker receives the same validated build identity. A waiting worker must identify a distinct target before activation; the tab then stores only the outgoing and expected identities. When that worker becomes the real controller, the app requests one guarded reload. The returning page consumes the content-free record and reports whether the expected application and controlling-worker identities agree. A separate disposable loopback gate compiles source, deliberately incomplete rejected-candidate, and intact-target production builds. Desktop and mobile WebKit require the incomplete installer to become redundant before waiting, retain the source controller and edited synthetic Draft, and still offer the later intact target through the full handoff. Same-origin tabs exchange only ephemeral presence, changed-work, action-pending, and file-review-pending flags; they never synchronize content, and changed work or an unsettled operation in another tab holds updates and fresh-session clearing here until that peer closes. Local Envelope identifiers route the Owner back to a pending review but never cross tabs. Disable makes the current workspace read-only, cancels transient reviews, and preserves accepted local documents, sources, Attachments, and Document Versions for inspection/download until refresh or confirmed fresh-session replacement. This does not inspect arbitrary cache responses or asset bytes, save, merge, migrate, recover, or durably delete state, recover or roll back an activated or controlling bad worker, prove native Safari or an installed app, or establish supported-browser installation/update behavior, so the v1 release gate remains unchecked.
