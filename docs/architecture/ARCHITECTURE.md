...
The PWA uses an injected-manifest Workbox worker so the application and worker compile the same validated build identity without moving update authority into UI code. The worker retains precaching, navigation fallback, old-precache cleanup, prompt-driven skip waiting, and client claim. One exact `MessageChannel` protocol returns only that public identity. The current app refuses an Update Handoff until the waiting worker reports a distinct target, then requests one guarded reload when that worker becomes the real controller so the returning app can compare the expected target with the controller's self-report. A registration-level listener ignores redundant installers and restores the update offer only when a later worker reaches `installed`, is actually waiting, and the page already has a controller; this closes a retry gap after one failed installation without moving activation authority into React. A disposable loopback harness compiles and serves source, deliberately incomplete rejected-candidate, and intact-target production artifacts and exercises both the rejected and successful lifecycles in desktop and mobile WebKit. This is bounded cross-build identity and one missing-precache-response rejection path, not arbitrary cache/asset-byte inspection, supply-chain attestation, state migration, installed-app proof, rollback, bad-worker recovery after control, native Safari, or supported-browser qualification.
