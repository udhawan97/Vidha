...
The PWA uses an injected-manifest Workbox worker so the application and worker compile the same validated build identity without moving update authority into UI code. The worker retains precaching, navigation fallback, old-precache cleanup, prompt-driven skip waiting, and client claim. One exact `MessageChannel` protocol returns only that public identity. The current app refuses an Update Handoff until the waiting worker reports a distinct target, then requests one guarded reload when that worker becomes the real controller so the returning app can compare the expected target with the controller's self-report. A disposable loopback harness compiles and serves separate source and target production artifacts and exercises that lifecycle in desktop and mobile WebKit. This is bounded cross-build identity evidence, not cache/asset-byte inspection, supply-chain attestation, state migration, installed-app proof, rollback, bad-worker recovery, native Safari, or supported-browser qualification.
