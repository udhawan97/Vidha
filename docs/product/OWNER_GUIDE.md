...
- Refresh, a confirmed app update, or a confirmed **Start fresh local rehearsal** action clears the browser session. After accepted session work, common reload paths show a browser warning and a waiting build requires **Update and clear session**. If that Update Handoff leaves this page alive for ten seconds or browser history returns it, the app restores the decision and reload warning. Before activation, the waiting worker must report a valid build identity distinct from the current application; the tab records only those two identities. When the accepted worker becomes the browser's controller, the app requests one guarded reload. A returning page compares its identity with the expected build and, when available, the controller's self-reported identity, then consumes that content-free record. The receipt alone does not inspect cache entries or asset bytes, recover a bad worker, qualify a browser, or preserve the rehearsal. The repository separately exercises source, deliberately incomplete rejected-candidate, and intact-target builds in disposable WebKit. The incomplete installer must become redundant before waiting while the source controller and edited Draft remain available; a later intact target must still be offered. That is not arbitrary corrupt-build coverage, native Safari, an installed app, or release evidence. The two confirmed clearing paths show live counts for this tab's edited documents, import sources, Attachments, Document Versions, undo/redo steps, and local Plan events, plus a **Review Envelope** route for affected Envelopes; neither the inventory nor the warning stores, recovers, synchronizes, or proves a download of anything. Disable keeps accepted local work available only as a read-only view until one of those clearing paths occurs.
...
9. Treat **Disable rehearsal** as terminal for that synthetic Plan. The ended Envelope workspace becomes read-only: unsettled file/restore work is canceled, edits and removals stay unavailable, and accepted documents, original files, Attachments, and Document Versions remain available for review or download. **Start fresh local rehearsal** explicitly clears the session before loading a separate Draft; it never resumes the Disabled Plan.
...
11. If a new build is waiting after you change the rehearsal, choose **Review update**. Download any portable copies or originals you need, then either **Keep working** or explicitly **Update and clear session**. Finish any in-flight Owner action or file preparation/approval first. An unresponsive or same-identity waiting worker keeps the update blocked. A candidate that fails installation before waiting is not offered at all; the current source page and rehearsal remain in place, and a later intact waiting worker may still be offered. If tab-scoped storage cannot record the outgoing and expected build identities, the update does not start. A rejected, timed-out, or browser-history-returned Update Handoff clears that record, leaves the rehearsal open, restores ordinary changed-work reload protection, and offers the decision again. If the accepted worker takes control, the app requests one guarded reload. After navigation, a one-time Build Transition Receipt reports whether the expected application and controlling-worker identities agree. It is not a cache/asset audit, rollback or recovery from a controlling bad worker, installed-app proof, supported-browser qualification, or release evidence.
