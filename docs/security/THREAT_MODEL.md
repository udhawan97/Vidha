...
No item above checks a v1 release gate. Browser WebAuthn/cookies, external key custody and authenticated database restore, integrated canonical scheduled-command crash evidence, rootless adversarial import isolation, providers, durable-volume recovery, failover, production sizing, and full outage evidence remain Phase 3B closure work or later.
