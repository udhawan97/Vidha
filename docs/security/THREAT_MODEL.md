...
**Status:** Pre-v1 security contract. Phase 3B supplies disposable executable evidence for selected WebAuthn, durable identity/recovery, wrapped metadata, PostgreSQL transaction/lease, restore-safe, and import-tool properties. Phase 3C adds a synthetic conversion-review and versioned Editable Document boundary; Phase 3D adds bounded in-memory Document Version and restore-planning evidence; Phase 3E adds an exact-state synthetic Draft Rehearsal Review; Phase 3F adds serialized local Owner actions and explicit terminal-session replacement; Phase 3G adds session-aware reload protection and an explicit service-worker update-loss decision; Phase 3H adds a content-free same-origin tab-presence boundary and holds two destructive local clearing paths while a peer reports changed work or an unsettled Owner action; Phase 3I withholds Draft rehearsal and Arm around unsettled file decisions and coordinates active file preparation with reload, update, and peer warning; Phase 3J makes a Disabled rehearsal read-only, preserves accepted local material for inspection/download, and cancels transient document work; Phase 3K adds a deterministic current-tab loss inventory and affected-Envelope return route to both confirmed clearing paths. Controls remain requirements until the exact runnable release candidate proves them.
...
- Phase 3G treats successful Plan actions and accepted or staged browser document work as a conservative signal that reload would discard session state. Common browser unload receives a native warning; a waiting service-worker build names the affected in-memory surfaces, cannot apply during an Owner action, and requires a safe-default confirmation. Rejection restores unload protection and leaves the dialog open. This does not save, migrate, encrypt, recover, or durably delete state, coordinate multiple tabs, prove bad-service-worker recovery, or establish supported-browser update behavior.
- Phase 3H uses a versioned same-origin `BroadcastChannel` protocol carrying only an ephemeral tab identity plus changed-work and Owner-action flags. Peer tabs ignore malformed, foreign-protocol, and self messages, expose that no state synchronizes, and hold update or fresh-session clearing while another tab reports changed work or an unsettled action. Pagehide/pageshow signaling and heartbeat expiry bound ordinary stale presence. This does not copy Plan, contact, Envelope, Editable Document, Attachment, filename, Recipient, or event data; persist or recover anything; merge concurrent work; cover every abrupt browser/OS failure; provide a cross-device or server lock; or establish supported-browser behavior.
- Phase 3I reports file-review preparation, approval, and ready-decision state above the Envelope workspace. Draft review and Arm remain unavailable while any decision is unsettled, active work receives `beforeunload` and app-update holds, and the Overview returns to the affected local Envelope. Peers receive only a file-review-pending boolean, with the existing changed-work bit set conservatively for Phase 3H compatibility; Envelope identifiers, filenames, bytes, MIME types, Recipients, documents, Attachments, events, and review results stay local. This is not upload, quarantine, malware scanning, durable workflow, cancellation, recovery, shared state, a server task, or a production lock.
- Phase 3J keeps the same local Envelope workspace mounted after Disable, invalidates active file reads and approvals, discards pending file decisions, closes restore review, and rejects document mutations in both controls and handlers. Accepted documents, imported-source bytes, Attachments, and Document Versions remain inspectable and downloadable until refresh, confirmed update, or confirmed fresh-session replacement. This is a synthetic in-memory presentation boundary, not persistence, an immutable archive, durable deletion, authentication, client trust, or server authorization.
- Phase 3K derives one current-tab session-loss inventory from the known synthetic baseline, current Envelopes and Plan events, and content-limited per-Envelope workspace-history counts. Confirmed app-update and fresh-session dialogs show exact counts and return links before clearing. The inventory does not persist or copy document/file content, observe other tabs, prove download completion or recovery, establish authorization, or replace the existing safe-default confirmation and peer/file/action holds.
