...
**Status:** Pre-v1 security contract. Phase 3B supplies disposable executable evidence for selected WebAuthn, durable identity/recovery, wrapped metadata, PostgreSQL transaction/lease, restore-safe, and import-tool properties. Phase 3C adds a synthetic conversion-review and versioned Editable Document boundary; Phase 3D adds bounded in-memory Document Version and restore-planning evidence; Phase 3E adds an exact-state synthetic Draft Rehearsal Review; Phase 3F adds serialized local Owner actions and explicit terminal-session replacement; Phase 3G adds session-aware reload protection and an explicit service-worker update-loss decision. Controls remain requirements until the exact runnable release candidate proves them.
