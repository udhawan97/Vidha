...
The [Phase 3B six-slice record](docs/product/NEXT_PHASE_3B_SIX.md) has intermediate disposable executable adapters for PostgreSQL topology, wrapped-key backup manifests, WebAuthn ceremonies, durable identity/recovery, integrated PostgreSQL Plan/outbox storage, and file/ClamAV/Pandoc intake. Closure slices 1–3 now add source gates for disposable topology failure/capacity/teardown, authenticated logical restore and atomic key rotation, and the loopback-only WebAuthn/session boundary; exact-commit CI acceptance remains mandatory. Phase 3B remains in progress: closure slices 4–6 in the [closure six](docs/product/PHASE_3B_CLOSURE_SIX.md) must still prove wired recovery abuse, canonical scheduled-command crashes, and rootless import isolation before accepting real data or sending a message. Guardian authority, Veto Window, Delivery Hold, Automatic Fallback, and Release remain outside the implemented state machine.
