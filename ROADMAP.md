...
- Exact current-tab loss review for confirmed clearing paths plus a bounded Update Handoff that restores reload protection and the Owner's decision when the tab remains alive or returns
- Validated application build identity plus a content-free Build Transition Receipt that distinguishes a changed-build return from an unverified same-build reload
- Disposable source-to-target production builds plus a real WebKit waiting-worker transition, guarded controller-change reload, and expected application/controller identity receipt
- Disposable source-to-rejected-to-target production builds that preserve the still-open synthetic rehearsal when an incomplete precache fails before waiting and still offer a later intact target
...
The [Phase 3L six-slice record](docs/product/NEXT_PHASE_3L_SIX.md) bounds the accepted service-worker Update Handoff, restores changed-work reload protection when the current page remains or returns, keeps stale asynchronous failures from overwriting the latest decision, and returns focus to the reversible choice. It does not verify build activation, recover or roll back a bad service worker, migrate session state, or establish supported-browser update behavior.
...
The [Phase 3O six-slice record](docs/product/NEXT_PHASE_3O_SIX.md) compiles separate disposable source and target production builds, moves the target worker through a real waiting-to-controlling transition, requests one guarded reload on `controllerchange`, and requires the returning target application and controller to agree with the expected content-free identity in desktop and mobile WebKit. It does not inspect every cache response or asset byte, exercise a corrupt target, recover or roll back a bad worker, migrate state, qualify native Safari or any supported browser, deploy the app, or authorize a release.

The [Phase 3P six-slice record](docs/product/NEXT_PHASE_3P_SIX.md) adds one deliberately incomplete, separately identified target between the source and intact target. Desktop and mobile WebKit require its installer to become redundant before waiting, retain the source controller and edited Draft, expose no destructive update action, and then offer a later intact target through the existing Update Handoff. It is not arbitrary cache or asset-byte integrity, recovery or rollback from an activated or controlling bad worker, state persistence, native Safari or supported-browser evidence, deployment, or release authority.
